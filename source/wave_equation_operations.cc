...
namespace HDG_WE
{
  ...
  InverseMassMatrixData<dim, fe_degree, Number>::InverseMassMatrixData(
  ...
  template <int dim, int fe_degree, typename Number>
  InverseMassMatrixData<dim, fe_degree, Number>::InverseMassMatrixData(
    const InverseMassMatrixData &other)
    : phi(other.phi)
    ...
    if (computing_times[2] > 0)
      {
        ...
        pcout << std::scientific << std::setw(9) << data.min << " (p" << std::setw(4)
              << data.min_index << ") " << std::setw(9) << data.avg << std::setw(9)
              << data.max << " (p" << std::setw(4) << data.max_index << ")" << std::endl;
        pcout << "   Inverse mass : ";
        ...
        if (computing_times[4] > 0)
          {
            ...
            pcout << std::scientific << std::setw(9) << data.min << " (p" << std::setw(4)
                  << data.min_index << ") " << std::setw(9) << data.avg << std::setw(9)
                  << data.max << " (p" << std::setw(4) << data.max_index << ")"
                  << std::endl;
          }
      ...
      {
        pcout << " call of invmass in apply          " << std::scientific << std::setw(4)
              << Utilities::MPI::max(computing_times[1], MPI_COMM_WORLD) << std::endl;
        pcout << " call of domain and faces in apply " << std::scientific << std::setw(4)
              << Utilities::MPI::max(computing_times[0], MPI_COMM_WORLD) << std::endl;
      }
  }
  ...
  template <int dim, int fe_degree>
  std::string
  WaveEquationOperation<dim, fe_degree>::Name()
  {
    return "Runge-Kutta";
  }

  template <int dim, int fe_degree>
  const MatrixFree<dim, typename WaveEquationOperationBase<dim>::value_type> &
  WaveEquationOperation<dim, fe_degree>::get_matrix_free() const
  {
    return data;
  }
  ...
  template <int dim, int fe_degree>
  TimeControl &
  WaveEquationOperation<dim, fe_degree>::get_time_control() const
  {
    return time_control;
  }
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::apply_ader(
  ...
  template <int dim, int fe_degree>
  unsigned int
  WaveEquationOperation<dim, fe_degree>::cluster_id(unsigned int) const
  {
    return -1;
  }

  template <int dim, int fe_degree>
  typename WaveEquationOperationBase<dim>::value_type
  WaveEquationOperation<dim, fe_degree>::time_step(unsigned int) const
  {
  ...
  template <int dim, int fe_degree>
  typename WaveEquationOperationBase<dim>::value_type
  WaveEquationOperation<dim, fe_degree>::speed_of_sound(int cell_index,
                                                        int vect_index) const
  {
  ...
  template <int dim, int fe_degree>
  std::string
  WaveEquationOperationADER<dim, fe_degree>::Name()
  {
    return "ADER";
  }
  ...
  template <int dim, int fe_degree>
  std::string
  WaveEquationOperationADERADCONFULL<dim, fe_degree>::Name()
  {
  ...
  template <int dim, int fe_degree>
  std::string
  WaveEquationOperationADERLTS<dim, fe_degree>::Name()
  {
    return "ADERLTS";
  }
  ...
  template <int dim, int fe_degree>
  unsigned int
  WaveEquationOperationADERLTS<dim, fe_degree>::cluster_id(unsigned int cell) const
  {
    return cluster_manager.cell_cluster_ids[cell];
  }

  template <int dim, int fe_degree>
  typename WaveEquationOperation<dim, fe_degree>::value_type
  WaveEquationOperationADERLTS<dim, fe_degree>::time_step(unsigned int cell) const
  {
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::communicate_flux_memory() const
  {
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::setup(
    ...
    const std::vector<unsigned int>            &vectorization_categories)
  {
    ...
    dummy.close();
    std::vector<const AffineConstraints<value_type> *> constraints(dof_handlers.size(),
                                                                   &dummy);
    ...
    // const_cast<DoFHandler<dim>
    // *>(dof_handlers[0])->renumber_dofs(renumbering);
    // additional_data.initialize_mapping = true;
    // data.reinit(mapping,dof_handlers,constraints,quadratures,additional_data);
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADER<dim, fe_degree>::setup(
    const MappingQGeneric<dim>                 &mapping,
    const std::vector<const DoFHandler<dim> *> &dof_handlers,
    const std::vector<Material>                &mats,
    const std::vector<unsigned int>            &vectorization_categories)
  {
    ...
    WaveEquationOperation<dim, fe_degree>::setup(mapping,
                                                 dof_handlers,
                                                 mats,
                                                 vectorization_categories);
        ...
        if (k > 0)
          {
            ...
            AssertDimension(stride * quad.size(),
                            shape_infos_embed[k - 1].data[0].shape_values_eo.size());
            AssertIndexRange(stride * quad.size(),
                             shape_infos_embed[k - 1].data[0].shape_hessians_eo.size() +
                               1);
            for (unsigned int i = 0; i < quad.size(); ++i)
              for (unsigned int q = 0; q < stride; ++q)
                shape_infos_embed[k - 1].data[0].shape_hessians_eo[i * stride + q] =
                  shape_infos_embed[k - 1].data[0].shape_values_eo[i * stride + q] *
                  (quad_embed.weight(q) / quad.weight(i));
          }
      }
  ...
  template <int dim, int fe_degree>
  WaveEquationOperationADERADCONFULL<dim, fe_degree>::WaveEquationOperationADERADCONFULL(
    TimeControl &time_control_in,
    Parameters  &parameters_in)
    : WaveEquationOperation<dim, fe_degree>(time_control_in, parameters_in)
  {}
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERADCONFULL<dim, fe_degree>::setup(
    const MappingQGeneric<dim>                 &mapping,
    const std::vector<const DoFHandler<dim> *> &dof_handlers,
    const std::vector<Material>                &mats,
    const std::vector<unsigned int>            &vectorization_categories)
  ...
  template <int dim, int fe_degree>
  WaveEquationOperationADERLTS<dim, fe_degree>::WaveEquationOperationADERLTS(
    TimeControl &time_control_in,
    Parameters  &parameters_in)
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::setup(
    const MappingQGeneric<dim>                 &mapping,
    const std::vector<const DoFHandler<dim> *> &dof_handlers,
    const std::vector<Material>                &mats,
    const std::vector<unsigned int> &)
  {
    ...
    WaveEquationOperationADER<dim, fe_degree>::setup(mapping,
                                                     dof_handlers,
                                                     mats,
                                                     new_vec_categors);
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::reset_data_vectors(
    const std::vector<Material> mats)
  {
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::local_apply_domain(
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::evaluate_cell(
    FEEvaluation<dim, fe_degree, fe_degree + 1, dim, value_type> &phi_v,
    ...
    const unsigned int                                            cell) const
  {
    ...
    for (unsigned int q = 0; q < phi_v.n_q_points; ++q)
      {
        const Tensor<1, dim, VectorizedArray<value_type>> pressure_gradient =
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::evaluate_inner_face(
    FEFaceEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type> &phi,
    FEFaceEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type> &phi_neighbor,
    const LinearAlgebra::distributed::Vector<value_type>                 &src,
    ...
    for (unsigned int q = 0; q < phi.n_q_points; ++q)
      {
        ...
        Tensor<1, dim + 1, VectorizedArray<value_type>> val_minus =
        ...
        VectorizedArray<value_type> lambda =
          tau_inv * (normal_v_plus + normal_v_minus + tau_plus * val_plus[dim] +
        ...
        val_plus[dim] = boundary_fac * c_sq_plus * rho_plus *
                        (-normal_v_plus + tau_plus * (lambda - val_plus[dim]));
        val_minus[dim] = boundary_fac * c_sq_minus * rho_minus *
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::evaluate_boundary_face(
    FEFaceEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type> &phi,
    const LinearAlgebra::distributed::Vector<value_type>                 &src,
    const unsigned int                                                    face,
    const value_type                                                      boundary_fac,
    LinearAlgebra::distributed::Vector<value_type>                       *dst) const
  {
    ...
    const VectorizedArray<value_type> c_sq =
      phi.read_cell_data(speeds) * phi.read_cell_data(speeds);
    const VectorizedArray<value_type> tau =
      1. / phi.read_cell_data(speeds) / phi.read_cell_data(this->densities);
    ...
    for (unsigned int q = 0; q < phi.n_q_points; ++q)
      {
        ...
        switch (boundary_id)
          {
            ...
            case 3: // absorbing wall - mimics an open domain by the first order
                    // absorbing condition
              {
                lambda = tau / (tau + 1. / c / rho) * p_plus +
                         1. / (tau + 1. / c / rho) * normal_v_plus;
                break;
              }
            default:
              Assert(
        ...
        val_plus[dim] =
          boundary_fac * c_sq * rho * (-normal_v_plus + tau * (lambda - p_plus));
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::local_apply_mass_matrix(
    const MatrixFree<dim, value_type> &,
    LinearAlgebra::distributed::Vector<value_type>       &dst,
    const LinearAlgebra::distributed::Vector<value_type> &src,
    const std::pair<unsigned int, unsigned int>          &cell_range) const
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        ...
        for (unsigned int i = 0; i < dofs_per_component; ++i)
          for (unsigned int d = 0; d < dim + 1; ++d)
            mass_matrix_data->phi[0].begin_dof_values()[d * dofs_per_component + i] *=
              mass_matrix_data->coefficients[i];
#else
        mass_matrix_data->inverse.apply(mass_matrix_data->coefficients,
                                        dim + 1,
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::apply(
    const LinearAlgebra::distributed::Vector<value_type> &src,
    LinearAlgebra::distributed::Vector<value_type>       &dst) const
  {
    Timer timer;
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERADCONFULL<dim, fe_degree>::apply_ader(
    const LinearAlgebra::distributed::Vector<value_type> &src,
    LinearAlgebra::distributed::Vector<value_type>       &dst) const
  {
    ...
    this->data.loop(&WaveEquationOperation<dim, fe_degree>::local_apply_domain,
                    &WaveEquationOperation<dim, fe_degree>::local_apply_face,
                    &WaveEquationOperation<dim, fe_degree>::local_apply_boundary_face,
                    static_cast<const WaveEquationOperation<dim, fe_degree> *>(this),
                    dst,
                    src,
                    true,
                    MatrixFree<dim, value_type>::DataAccessOnFaces::values,
  ...
  namespace PostProcessingHelper
  {
    ...
    public:
      ...
      void
      vmult(VectorizedArray<Number> *dst, const VectorizedArray<Number> *src) const
      {
      ...
      void
      precondition(VectorizedArray<Number> *dst, const VectorizedArray<Number> *src) const
      {
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::compute_post_pressure(
    const LinearAlgebra::distributed::Vector<value_type> &solution,
    ...
    LinearAlgebra::distributed::Vector<value_type>       &post_pressure_vector) const
  {
    ...
    FEEvaluation<dim, fe_degree + 1, fe_degree + 2, 1, value_type> post_pressure(data,
                                                                                 1,
                                                                                 1,
                                                                                 0);
    AlignedVector<VectorizedArray<value_type>> rhs(post_pressure.dofs_per_cell),
      result(post_pressure.dofs_per_cell);
    PostProcessingHelper::MatrixPostProcessing<dim, fe_degree + 1, value_type> matrix(
      post_pressure);
    IterativeHelper::SolverCGvect<VectorizedArray<value_type>> cg_solver(
      post_pressure.dofs_per_cell,
      std::max(1e-20,
               static_cast<double>(1000. *
                                   std::sqrt(std::numeric_limits<value_type>::min()))),
      std::max(1e-11, 1e2 * std::numeric_limits<value_type>::epsilon()),
      6 * post_pressure.dofs_per_cell);
    ...
    for (unsigned int cell = 0; cell < data.n_cell_batches(); ++cell)
      {
        ...
        const VectorizedArray<value_type> preliminary_pressure =
          pressure.integrate_value();
        const VectorizedArray<value_type> adjust_value =
          (correct_pressure - preliminary_pressure) / volume;
  ...
  namespace
  {
    ...
    Tensor<1, dim, Number>
    make_tensor(const Tensor<1, dim, Number> &in)
    {
      return in;
    }
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::estimate_error(
    const LinearAlgebra::distributed::Vector<value_type> &solution,
    LinearAlgebra::distributed::Vector<value_type>       &tmp_vector,
    ...
    for (unsigned int cell = 0; cell < data.n_cell_batches(); ++cell)
      {
        ...
        for (unsigned int q = 0; q < pressure.n_q_points; ++q)
          pressure.submit_value(
            (pressure.get_gradient(q) - make_tensor(velocity.get_value(q))).norm_square(),
            q);
        const VectorizedArray<value_type> errors = pressure.integrate_value();
        for (unsigned int v = 0; v < data.n_active_entries_per_cell_batch(cell); ++v)
          post_pressure_vector(data.get_cell_iterator(cell, v)->active_cell_index()) =
            std::sqrt(errors[v]);
      }
  }
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperation<dim, fe_degree>::project_initial_field(
    LinearAlgebra::distributed::Vector<value_type> &solution,
    const Function<dim>                            &function) const
  {
    const unsigned int n_q_points =
      FEEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type>::static_n_q_points;
    FEEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type> &phi =
      mass_matrix_data->phi[0];
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADER<dim, fe_degree>::local_apply_firstader_domain(
    const MatrixFree<dim, value_type> &,
    LinearAlgebra::distributed::Vector<value_type>       &dst,
    const LinearAlgebra::distributed::Vector<value_type> &src,
    const std::pair<unsigned int, unsigned int>          &cell_range) const
  {
    ...
    for (unsigned int cell = cell_range.first; cell < cell_range.second; ++cell)
      {
        integrate_taylor_cauchykovalewski(
          cell, phi_eval, src, this->time_control.get_time_step(), 0.0, 0.0, dst);
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADER<dim, fe_degree>::integrate_taylor_cauchykovalewski(
    ...
    const LinearAlgebra::distributed::Vector<value_type>             &recongraddiv) const
  {
    ...
    phi_eval.gather_evaluate(src,
                             EvaluationFlags::values | (this->parameters.use_ader_post ?
                                                          EvaluationFlags::nothing :
                                                          EvaluationFlags::gradients));
    for (unsigned int q = 0; q < n_q_points; ++q)
      {
        // contribution from k=0
        const Tensor<1, dim + 1, VectorizedArray<value_type>> v_and_p =
          phi_eval.get_value(q);
        for (unsigned int d = 0; d < dim; ++d)
        ...
        if (!(this->parameters.use_ader_post))
          {
            ...
            const Tensor<1, dim + 1, Tensor<1, dim, VectorizedArray<value_type>>>
              v_and_p_grad    = phi_eval.get_gradient(q);
            ...
            for (unsigned int d = 0; d < dim; ++d)
              contrib[d * n_q_points + q] +=
                ((t2 - te) * (t2 - te) - (t1 - te) * (t1 - te)) * 0.5 * v_and_p_next[d];
            contrib[dim * n_q_points + q] +=
              ((t2 - te) * (t2 - te) - (t1 - te) * (t1 - te)) * 0.5 * v_and_p_next[dim];
    ...
    integrate_taylor_cauchykovalewski_step<0, true>(
      cell, phi_eval, phi_eval.begin_values(), t1 - te, t2 - te, contrib);

    // this operation corresponds to three steps:
    // phi_eval.submit_value();
    // phi_eval.integrate();
    // inverse.apply();
    this->mass_matrix_data->inverse.transform_from_q_points_to_basis(
      dim + 1, contrib, phi_eval.begin_dof_values());
  }
  ...
  void
  WaveEquationOperationADER<dim, fe_degree>::integrate_taylor_cauchykovalewski_step(
    const unsigned int                                                cell,
    FEEvaluation<dim, fe_degree, fe_degree + 1, dim + 1, value_type> &phi_eval,
    ...
    VectorizedArray<value_type>                                      *contrib) const
  {
    ...
    for (int k = 2; k <= step_no + 2; ++k)
      fac /= -(k + 1);
    value_type fac_t = fac * (Utilities::fixed_power<step_no + 3>(tend) -
                              Utilities::fixed_power<step_no + 3>(tstart));

    constexpr int reduce_step      = step_no / 2;
    constexpr int reduce_degree_by = 2 * reduce_step;
    bool          at_reduce_step   = ((step_no - 2 * reduce_step) == 1) &&
                          (this->parameters.spectral_evaluation == true);
    ...
    if (this->parameters.spectral_evaluation)
      {
        ...
        if (this->data.get_mapping_info().get_cell_type(cell) ==
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              {
                ...
                if (!add_into_contrib)
                  {
                    for (unsigned int d = 0; d < dim; ++d)
                      contrib[d * n_q_points + q] =
                        (fac_t * rho_inv) * spectral_array[q + (dim + d) * n_q_points] *
                        jac[d][d];
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  spectral_array[q + d * n_q_points] =
                    rho_inv * spectral_array[q + (dim + d) * n_q_points] * jac[d][d];
        ...
        else
          {
            ...
            const unsigned int stride =
              (this->data.get_mapping_info().get_cell_type(cell) ==
            ...
            for (unsigned int q = 0; q < n_q_points; ++q)
              {
                ...
                const Tensor<2, dim, VectorizedArray<value_type>> &jac =
                  this->data.get_mapping_info()
                    .cell_data[index]
                    .jacobians[0][this->data.get_mapping_info()
                                    .cell_data[index]
                                    .data_index_offsets[cell] +
                                  q * stride];
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  for (unsigned int e = 0; e < dim; ++e)
                    v_divergence +=
                      spectral_array[q + (d * dim + e) * n_q_points] * jac[d][e];
                ...
                for (unsigned int d = 0; d < dim; ++d)
                  {
                    ...
                    for (unsigned int e = 1; e < dim; ++e)
                      p_grad[d] +=
                        spectral_array[q + (dim * dim + e) * n_q_points] * jac[d][e];
                  }
    ...
    else
      {
        ...
        //{
        this->mass_matrix_data->inverse.apply(this->mass_matrix_data->coefficients,
                                              dim + 1,
        ...
        if (at_reduce_step)
          {
            ...
            internal::FEEvaluationImplBasisChange<internal::evaluate_evenodd,
                                                  internal::EvaluatorQuantity::hessian,
                                                  dim,
                                                  next_degree + 1,
                                                  my_degree + 1>::
              do_backward(dim,
                          shape_infos_embed[reduce_step].data[0].shape_hessians_eo,
                          false,
                          spectral_array,
                          spectral_array);
            VectorizedArray<value_type>
              next_contrib_array[Utilities::pow(next_degree + 1, dim) * (dim + 1)];

            // run Taylor-Cauchy-Kovalewski at lower degree
            this->template integrate_taylor_cauchykovalewski_step<
              (step_no < fe_degree - 2 ? step_no + 1 : step_no),
              false>(cell, phi_eval, spectral_array, tstart, tend, next_contrib_array);
            ...
            internal::FEEvaluationImplBasisChange<internal::evaluate_evenodd,
                                                  ...
                                                  my_degree + 1>::
              do_forward(dim,
                         shape_infos_embed[reduce_step].data[0].shape_values_eo,
                         next_contrib_array,
                         spectral_array);
        ...
        else
          {
            this->template integrate_taylor_cauchykovalewski_step<
              (step_no < fe_degree - 2 ? step_no + 1 : step_no),
              true>(cell, phi_eval, spectral_array, tstart, tend, contrib);
          }
      }
  }
  ...
  WaveEquationOperationADER<dim, fe_degree>::local_apply_ader_face(
    ...
    const std::pair<unsigned int, unsigned int>          &face_range) const
  {
    // basically the same as local_apply_face, but different signs in some
    // places
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADER<dim, fe_degree>::local_apply_secondader_domain(
    const MatrixFree<dim, value_type> &,
    LinearAlgebra::distributed::Vector<value_type>       &dst,
    const LinearAlgebra::distributed::Vector<value_type> &src,
    const std::pair<unsigned int, unsigned int>          &cell_range) const
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADER<dim, fe_degree>::apply_ader(
    const LinearAlgebra::distributed::Vector<value_type> &src,
    LinearAlgebra::distributed::Vector<value_type>       &dst) const
  {
    Timer timer;
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::local_apply_firstader_domain(
    const MatrixFree<dim, value_type> &,
    LinearAlgebra::distributed::Vector<value_type>       &dst,
    const LinearAlgebra::distributed::Vector<value_type> &src,
    const std::pair<unsigned int, unsigned int>          &cell_range) const
        ...
        if (cluster_manager.is_evaluate_cell(cell))
          {
            ...
            this->integrate_taylor_cauchykovalewski(
              cell, phi_eval, src, t2, t1, te, cluster_manager.improvedgraddiv);
            phi_eval.set_dof_values(dst);
          }
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::local_apply_secondader_domain(
    const MatrixFree<dim, value_type> &,
    LinearAlgebra::distributed::Vector<value_type>       &dst,
    const LinearAlgebra::distributed::Vector<value_type> &src,
    const std::pair<unsigned int, unsigned int>          &cell_range) const
        ...
        if (cluster_manager.is_update_cell(cell))
          {
            ...
            this->integrate_taylor_cauchykovalewski(
              cell, phi_eval, src, dt, 0., 0., cluster_manager.improvedgraddiv);
            ...
            // now comes the update!
            {
              ...
              const VectorizedArray<value_type> rho_inv = 1. / this->densities[cell];
              const VectorizedArray<value_type> c_sq =
                this->speeds[cell] * this->speeds[cell];
              ...
              for (unsigned int q = 0; q < n_q_points; ++q)
                {
                  ...
                  Tensor<1, dim + 1, Tensor<1, dim, VectorizedArray<value_type>>>
                    temp_gradient;
                  for (unsigned int d = 0; d < dim; ++d)
              ...
              if (cluster_manager.is_fluxmemory_considered)
                {
                  ...
                  help_eval.set_dof_values(
                    flux_memory); // tell the flux_memory variable, that some of
                                  // its values are reset
                }
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::local_apply_dummy_domain(
    const MatrixFree<dim, value_type> &,
        ...
        if (cluster_manager.is_evaluate_face(face))
          {
            ...
            phi.distribute_local_to_global(flux_memory,
                                           0,
                                           cluster_manager.get_phi_to_fluxmemory(face));
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::evaluate_cells_and_faces_first_ader(
    const LinearAlgebra::distributed::Vector<value_type> &src,
    LinearAlgebra::distributed::Vector<value_type>       &dst) const
  {
    this->data.cell_loop(
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::evaluate_cells_second_ader(
    const LinearAlgebra::distributed::Vector<value_type> &src,
    LinearAlgebra::distributed::Vector<value_type>       &dst) const
  {
    this->data.cell_loop(
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::apply_ader(
    const LinearAlgebra::distributed::Vector<value_type> &src,
    LinearAlgebra::distributed::Vector<value_type>       &dst) const
  {
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::reconstruct_div_grad(
    const LinearAlgebra::distributed::Vector<value_type> &src,
    LinearAlgebra::distributed::Vector<value_type>       &dst) const
  {
    this->data.loop(
      &WaveEquationOperationADERLTS<dim, fe_degree>::local_apply_postprocessing_domain,
      &WaveEquationOperationADERLTS<dim, fe_degree>::local_apply_postprocessing_face,
      &WaveEquationOperationADERLTS<dim,
                                    fe_degree>::local_apply_postprocessing_boundary_face,
      this,
      dst,
      src);
    ...
    this->data.cell_loop(
      &WaveEquationOperationADERLTS<dim,
                                    fe_degree>::local_apply_postprocessing_mass_matrix,
      this,
      dst,
      dst);
  }
  ...
  template <int dim, int fe_degree>
  void
  WaveEquationOperationADERLTS<dim, fe_degree>::local_apply_postprocessing_mass_matrix(
    const MatrixFree<dim, value_type> &,
    LinearAlgebra::distributed::Vector<value_type>       &dst,
    const LinearAlgebra::distributed::Vector<value_type> &src,
    const std::pair<unsigned int, unsigned int>          &cell_range) const
        ...
        if (cluster_manager.is_evaluate_cell(cell))
          {
            ...
            this->mass_matrix_data->inverse.fill_inverse_JxW_values(
              this->mass_matrix_data->coefficients);
            this->mass_matrix_data->inverse.apply(
              this->mass_matrix_data->coefficients,
              dim + 1,
  ...
  // explicit instaniation for all operators for space dimensions 2,3 and
  // polynomial degrees 1,...,12
