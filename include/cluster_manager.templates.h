...
namespace HDG_WE
{
  ...
  template <int dim>
  void
  ClusterManager<Number>::iterate_cluster_categorization(
    ...
    const double                                cfl_number)
  {
    ...
    for (; cell != endc; ++cell) // if (cell->is_locally_owned())
      if (!cell->is_artificial())
        {
          timestepcollection[cell->active_cell_index()] =
            cfl_number * cell->minimum_vertex_distance();
              ...
              if (!cell->is_artificial() && !cell->is_ghost())
                {
                  if (temporary_cluster_ids[cell->active_cell_index()] == c)
                    ...
                    } // if(temporary_cluster_ids[cell->active_cell_index()] ==
                      // c)
                }     // for (; cell!=endc; ++cell)
          }           // for (unsigned int c=0; c<n_clusters; ++c)
  ...
  template <int dim>
  void
  ClusterManager<Number>::propose_cluster_categorization(
    ...
    const double                                cfl_number)
  {
    this->iterate_cluster_categorization(
      tria, dof_handlers, max_clusters, max_diff, cfl_number);

    // cast triangulation
    parallel::distributed::Triangulation<dim> *triapll =
      (dynamic_cast<parallel::distributed::Triangulation<dim> *>(
        const_cast<dealii::Triangulation<dim> *>(&tria)));
    ...
    for (unsigned int i = 0; i < dof_handlers.size(); ++i)
      (const_cast<dealii::DoFHandler<dim> *>(dof_handlers[i]))
        ->distribute_dofs(dof_handlers[i]->get_fe());

    // setup the cluster proposition according to the new cell distribution -
    // don't try anything too smart, just do it again
    this->iterate_cluster_categorization(
      tria, dof_handlers, max_clusters, max_diff, cfl_number);
  ...
  template <typename Operator>
  void
  ClusterManager<Number>::setup(const Operator &op)
  {
    ...
    evaluate_face.resize(op.get_matrix_free().n_inner_face_batches() +
                         ...
                         false);
    phi_to_dst.resize(op.get_matrix_free().n_inner_face_batches() +
                      op.get_matrix_free().n_boundary_face_batches());
    phi_neighbor_to_dst.resize(op.get_matrix_free().n_inner_face_batches() +
                               op.get_matrix_free().n_boundary_face_batches());
    ...
    // for a general clustering, we have to check the update criteria for each
    // cluster restriction is: neighboring clusters are only one apart
    ...
    op.get_time_control().set_time_step(fastest_time_step *
                                        (cluster_diff * (n_clusters - 1) + 1));

    n_updates = cluster_update_order.size();

    for (unsigned c = 0; c < cluster_update_times.size(); ++c)
      if (cluster_update_times[c][1] >
          (cluster_diff * (n_clusters - 1) + 1) * fastest_time_step)
        cluster_update_times[c][1] =
          (cluster_diff * (n_clusters - 1) + 1) * fastest_time_step;
    ...
    //  std::cout<<"c "<<c<<" updatecluster
    //  "<<cluster_update_order[c]<<std::endl;

    // minimizing the calls to get_cell_iterator in update_cells by storing
    // respective indices beforehand
      ...
      for (unsigned int v = 0; v < n_vect; ++v)
        {
          mf_faceinfo_cellsminus[v][f] =
            op.get_matrix_free().get_face_info(f).cells_interior[v];
          if (f < op.get_matrix_free().n_inner_face_batches())
            mf_faceinfo_cellsplus[v][f] =
              op.get_matrix_free().get_face_info(f).cells_exterior[v];
        }
  }
  ...
  template <typename Operator>
  void
  ClusterManager<Number>::perform_time_step(
    const Operator                                   &op,
    ...
    for (unsigned int cycle = 0; cycle < n_updates; ++cycle)
      {
        ...
        // in case we want superconvergence, we need to do the reconstruction
        // step as explained in the paper
        if (use_ader_post)
          {
            // init
            ...
            // which cells do we have to update to be able to perform
            // reconstruction all cells who are neighbor of actual cluster and
            // have lower or higher time level start with faster cluster:
            ...
            if (actual_cluster > 0)
              if (std::abs(actual_cluster_time -
                  ...
                  relative_tolerance * fastest_time_step)
                {
                  for (unsigned int e = 0; e < n_cells_with_ghosts; ++e)
                    {
                      if (cell_cluster_ids[e] == actual_cluster - 1 &&
                          cell_have_slower_neighbor[e])
                        temp_faster[e] = true;
                      else
                        temp_faster[e] = false;
                    }
                  // expand this selection by the neighbors of those already set
                  // (later we need neighbor of neighbor info!)
                  for (unsigned int e = 0; e < n_cells_with_ghosts; ++e)
                    update_cell[e] = false;
                  for (unsigned int e = 0; e < n_cells_with_ghosts; ++e)
                    {
                  ...
                  update_elements(
                    op, dst, temporary_recon_state, actual_cluster - 1, false);
                }
                          ...
                          if (evaluate_cell[mf_faceinfo_cellsminus[v][f] /
                                            ...
                                            Operator::n_vect])
                            {
                              ...
                              if (evaluate_cell[mf_faceinfo_cellsminus[v][f] /
                                                Operator::n_vect])
                                phi_to_dst[f][v] = true;
                              if (evaluate_cell[mf_faceinfo_cellsplus[v][f] /
                                                Operator::n_vect])
  ...
  template <typename Operator>
  void
  ClusterManager<Number>::update_elements(
    const Operator                             &op,
    ...
    const bool                                  write_to_fluxmemory) const
  {
    ...
    for (unsigned int e = 0; e < n_cells_with_ghosts; ++e)
      {
        if (update_cell[e] == true && cell_cluster_ids[e] != actual_cluster)
          Assert(
            false,
            ExcMessage(
              "it is not allowed to call update elements on cells of differing cluster!"));
    ...
    if (actual_cluster > 0)
      {
        // prepare everything for face evaluation between actual_cluster and
        // actual_cluster-1
        t1 = t1fa;
        t2 = t2fa;

        if (std::abs(t2 - t1) >
            relative_tolerance * fastest_time_step) // only do this if necessary
          {
            // setup the element list that need evaluation (all of update_cell
            // who have faster neighbor and all of actual_cluster-1 who are
            // neighbor of update_cell)
            for (unsigned int e = 0; e < n_cells_with_ghosts; ++e)
              {
                if (cell_cluster_ids[e] == actual_cluster &&
    ...
    if (actual_cluster < n_clusters - 1)
      {
        ...
        if (std::abs(t2 - t1) >
            relative_tolerance * fastest_time_step) // only do this if necessary
          {
            // setup the element list that need evaluation (all of
            // actual_cluster who have slower neighbor and all of
            // actual_cluster+1 who have faster neighbor)
            for (unsigned int e = 0; e < n_cells_with_ghosts; ++e)
              {
                if (cell_cluster_ids[e] == actual_cluster &&
