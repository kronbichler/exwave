...
namespace HDG_WE
{
  ...
  public:
    ...
    template <int dim>
    void
    propose_cluster_categorization(
    ...
    template <int dim>
    void
    iterate_cluster_categorization(
      const Triangulation<dim>                   &tria,
      const std::vector<const DoFHandler<dim> *> &dof_handlers,
      const unsigned int                          max_clusters,
      const unsigned int                          max_diff,
    ...
    template <typename Operator>
    void
    setup(const Operator &op);

    template <typename Operator>
    void
    setup_mf_index_to_cell_index(const Operator &op);
    ...
    bool
    is_evaluate_cell(unsigned int cell) const
    {
      return evaluate_cell[cell];
    }

    bool
    is_update_cell(unsigned int cell) const
    {
      return update_cell[cell];
    }

    bool
    is_evaluate_face(unsigned int face) const
    {
    ...
    Number
    get_te(unsigned int cell) const
    {
    ...
    get_phi_to_dst(unsigned int face) const
    {
      return phi_to_dst[face];
    }

    std::bitset<VectorizedArray<Number>::size()>
    get_phi_to_fluxmemory(unsigned int face) const
    {
      return phi_to_fluxmemory[face];
    }

    std::bitset<VectorizedArray<Number>::size()>
    get_phi_neighbor_to_dst(unsigned int face) const
    {
      return phi_neighbor_to_dst[face];
    }

    std::bitset<VectorizedArray<Number>::size()>
    get_phi_neighbor_to_fluxmemory(unsigned int face) const
    {
    ...
    // vectors of length cells with indicator for neighbors with smaller/bigger
    // time step size
  ...
  private:
    ...
    template <typename Operator>
    void
    update_elements(const Operator                             &op,
