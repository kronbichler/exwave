...
namespace HDG_WE
{
  ...
  // create your materials in the vector mats and assign the material values
  // (must conform with the material_ids you set later)
  std::vector<Material>
  input_materials();

  // set grid transform factor
  void
  set_grid_transform_factor(double in);

  // get grid transform factor
  double
  get_grid_transform_factor();

  // if you want to modify a cartesian grid by a function, fill in the formula
  // here
  template <int dim>
  Point<dim>
  ...
  // after creating the geometry, set boundary ids! they define the type of
  // boundary conditions 1 - soft wall - normal velocity component is zero 2 -
  // hard wall - pressure is zero 3 - absorbing wall - mimics an open domain by
  // the first order absorbing condition
  template <int dim>
  void
  input_geometry_description(Triangulation<dim> &tria, const Parameters &parameters)
  {
        ...
        case 1:
          {
            GridGenerator::subdivided_hyper_cube(tria,
                                                 parameters.n_initial_intervals,
                                                 0,
                                                 1);
  ...
  // define the "exact solution" (first dim components are for velocity, last
  // for the pressure) this function is also used for the initial conditions
  template <int dim>
  double
  input_exact_solution(const Point<dim>  &p,
                       const double       t,
                       const unsigned int component,
                       const bool         time_derivative,
            ...
            if (time_derivative)
              {
                if (component == dim)
                  return_value =
                    -membrane_modes * std::sqrt(dim) * numbers::PI *
                    std::sin(membrane_modes * std::sqrt(dim) * numbers::PI * t);
                else
                  return_value =
                    -membrane_modes * numbers::PI *
                    std::cos(membrane_modes * std::sqrt(dim) * numbers::PI * t);
              }
            else
              {
                if (component == dim)
